...
!function(){
  ...
  if( webOS.platform = {}, window.PalmSystem ){
    if( navigator.userAgent.indexOf( 'SmartWatch' ) > -1 ){
      ...
      try {
        var e = JSON.parse( PalmSystem.deviceInfo || '{}' );if( e.platformVersionMajor && e.platformVersionMinor ){
          var t = parseInt( e.platformVersionMajor ), o = parseInt( e.platformVersionMinor );t < 3 || 3 == t && o <= 0 ? webOS.platform.legacy = !0 : webOS.platform.open = !0
        }
      }
      catch ( i ){
        webOS.platform.open = !0
      }window.Mojo = window.Mojo || { relaunch:function( e ){} }, window.PalmSystem && PalmSystem.stageReady && PalmSystem.stageReady()
    }
  }
  else {
  ...
  webOS.fetchAppId = function(){
    ...
    } var o = e.match( new RegExp( '.*://[^#]*/' ) );return o ? o[0] : ''
    ...
    if( this.device ){
      ...
      this.device = {};try {
        ...
        if( i.device.modelName = t.modelName || i.device.modelName, i.device.modelNameAscii = t.modelName || i.device.modelNameAscii, i.device.sdkVersion = t.sdkVersion || i.device.sdkVersion, i.device.uhd = 'true' === t.UHD, t.firmwareVersion && '0.0.0' !== t.firmwareVersion || ( t.firmwareVersion = t.sdkVersion ), t.firmwareVersion ){
          i.device.version = t.firmwareVersion;for ( var o = i.device.version.split( '.' ), s = ['versionMajor', 'versionMinor', 'versionDot'], n = 0;n < s.length;n++ ){
            try {
  ...
  webOS.feedback = { play:function( e ){
    if( webOS && webOS.platform && webOS.platform.watch ){
      var t = { name:e || 'touch', sink:'pfeedback' };if( !window.PalmServiceBridge ){
        return;
      }webOS.service.request( 'luna://com.palm.audio/systemsounds', { method:'playFeedback', parameters:t, subscribe:!1, resubscribe:!1 } )
    }
  } }
}(), function(){
  webOS.keyboard = { isShowing:function(){
    return !!( PalmSystem && PalmSystem.isKeyboardVisible && PalmSystem.isKeyboardVisible() )
  } }
}(), function(){
  webOS.notification = { showToast:function( e, t ){
    var o = e.message || '', i = e.icon || '', s = webOS.fetchAppId(), n = e.appId || s, r = e.appParams || {}, a = e.target, c = e.noaction, l = e.stale || !1, m = e.soundClass || '', u = e.soundFile || '', d = e.soundDurationMs || '';if( webOS.platform.legacy || webOS.platform.open ){
      var f = ( e.response || { banner:!0 }, PalmSystem.addBannerMessage( o, JSON.stringify( r ), i, m, u, d ) );t && t( f )
    }
    else {
      o.length > 60 && console.warn( 'Toast notification message is longer than recommended. May not display as intended' );var b = { sourceId:s, message:o, stale:l, noaction:c };i && i.length > 0 && ( b.iconUrl = i ), c || ( a ? b.onclick = { target:a } : b.onclick = { appId:n, params:r } ), this.showToastRequest = webOS.service.request( 'palm://com.webos.notification', { method:'createToast', parameters:b, onSuccess:function( e ){
        t && t( e.toastId )
    ...
    if( webOS.platform.legacy || webOS.platform.open ){
      ...
      this.removeToastRequest = webOS.service.request( 'palm://com.webos.notification', { method:'closeToast', parameters:{ toastId:e } } )
    }
  }, supportsDashboard:function(){
    return webOS.platform.legacy || webOS.platform.open
  }, showDashboard:function( e, t ){
    if( webOS.platform.legacy || webOS.platform.open ){
      var o = window.open( e, '_blank', 'attributes={"window":"dashboard"}' );return t && o.document.write( t ), o.PalmSystem && o.PalmSystem.stageReady(), o
  ...
  function e( e, t ){
    ...
    if( !window.PalmServiceBridge ){
      return this.onFailure && this.onFailure( { errorCode:-1, errorText:'PalmServiceBridge not found.', returnValue:!1 } ), this.onComplete && this.onComplete( { errorCode:-1, errorText:'PalmServiceBridge not found.', returnValue:!1 } ), void console.error( 'PalmServiceBridge not found.' );
    } this.bridge = new PalmServiceBridge();var t = this;this.bridge.onservicecallback = this.callback = function( o ){
      var i;if( !t.cancelled ){
        try {
          i = JSON.parse( o )
        }
        catch ( s ){
    ...
    return '[LS2Request]'
  }, e.resubscribeDelay = 1e4, webOS.service = { request:function( t, o ){
    return new e( t, o )
  }, systemPrefix:'com.webos.', protocol:'luna://' }, navigator.service = { request:webOS.service.request }, navigator.service.Request = navigator.service.request
}(), function(){
  webOS.libVersion = '0.1.0'
}(), function(){
  webOS.voicereadout = { readAlert:function( e, t ){
    var o = 'boolean' !== typeof t || t;if( webOS && webOS.platform && webOS.platform.watch ){
      var i, s, n = function( e ){
            webOS.service.request( 'luna://com.webos.settingsservice', { method:'getSystemSettings', parameters:{ category:'VoiceReadOut' }, onSuccess:function( t ){
              t && t.settings.talkbackEnable && e()
            }, onFailure:function( e ){
              console.error( 'Failed to get system VoiceReadOut settings: ' + JSON.stringify( e ) )
            } } )
          }, r = function( e ){
            webOS.service.request( 'luna://com.webos.settingsservice', { method:'getSystemSettings', parameters:{ keys:['localeInfo'] }, onSuccess:function( t ){
              i = t.settings.localeInfo.locales.TTS, e()
            }, onFailure:function( e ){
              console.error( 'Failed to get system localeInfo settings: ' + JSON.stringify( e ) )
            } } )
          }, a = function( e ){
            webOS.service.request( 'luna://com.webos.settingsservice', { method:'getSystemSettings', parameters:{ category:'option', key:'ttsSpeechRate' }, onSuccess:function( t ){
              s = Number( t.settings.ttsSpeechRate ), e()
            }, onFailure:function( e ){
              console.error( 'Failed to get system speechRate settings: ' + JSON.stringify( e ) )
            } } )
          }, c = function(){
            webOS.service.request( 'luna://com.lge.service.tts', { method:'speak', parameters:{ locale:i, text:e, speechRate:s } } )
          };n( function(){
        r( function(){
          a( c )
        } )
      ...
      var l = function( e ){
            webOS.service.request( 'luna://com.webos.settingsservice', { method:'getSystemSettings', parameters:{ keys:['audioGuidance'], category:'option' }, onSuccess:function( t ){
              t && 'on' === t.settings.audioGuidance && e()
            }, onFailure:function( e ){
              console.error( 'Failed to get system AudioGuidance settings: ' + JSON.stringify( e ) )
            } } )
          }, c = function(){
            webOS.service.request( 'luna://com.webos.service.tts', { method:'speak', parameters:{ text:e, clear:o }, onFailure:function( e ){
